#This is the closest to a final version of the integrated GUI with CV tracking and serial reading.
#Need to fix the contour tracking to iGUI.py and distance calculation to dispacement
...
class CoagulexApp:
    ...
    def update_video(self):
        """Update both video feeds with corrected variable references"""
        ...
        # Display first video feed with tracking
        if ret1:
            if self.monitoring_active:
                frame1 = self.process_contours(frame1.copy())
        ...
        # Display second video feed with tracking
        if ret2:
            if self.monitoring_active:
                frame2 = self.process_contours(frame2.copy())
